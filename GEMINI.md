...
- **Radix portals nest dialogs inside ID'd wrapper divs.** New AG renders project/model/worktree/branch popovers as `body > div#:rXX: > div[role="dialog"]`, not as direct body children. `capture.js` already handles this (searches inside `child.id` wrappers). `click-main.js` dialog source must do the same — use `child.querySelector('[role="dialog"]')` when `child.id` is set, mirroring the dropdown source pattern.
- **`suppressOverlay` window must be short (≤300ms).** The suppress exists to prevent a stale snapshot from re-opening the overlay right after the user dismisses it via backdrop. The actual stale-snapshot window is ≤100ms (one CDP round-trip for `/dismiss-portal`). A window >300ms causes "every other open" failures from remote because users naturally re-open dropdowns within 2 seconds of dismissing.
- **Tailwind v4 `@layer` kills captured utilities.** AG uses Tailwind v4 which wraps all utilities in `@layer` declarations. CSS cascade spec says unlayered styles beat layered styles regardless of specificity. AG2R's unlayered `* { margin: 0 }` reset would kill all captured `@layer utilities`. Fix: `capture.js` unwraps `@layer` blocks during CSS collection (step 13), and `style.css` has defense-in-depth fallbacks scoped to `#chat-content`.
